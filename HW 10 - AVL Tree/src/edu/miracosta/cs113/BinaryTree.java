...
public class BinaryTree<E> implements Serializable {
    ...
    public int getHeight ( Node < E > root ) {
        int leftHeight, rightHeight; // The heights of the left and right subtrees.
        if ( root == null )
            return 0;

        leftHeight  = 1 + getHeight ( root.left );  // Get the left subtree height ( plus 1 for root ).
        rightHeight = 1 + getHeight ( root.right ); // Get the right subtree height ( plus 1 for root ).
