...
public class PalindromeTest {
    ...
    private boolean isPalindrome(String s) {
        ...
        ArrayListStack<Character> forwardString = new ArrayListStack<> ();
        ArrayListStack<Character> backwardString = new ArrayListStack<> ();

        // Check if the String is null.
        if ( s == null )
            throw new IllegalArgumentException ( );

        // If the String is one character, or empty space, then it is a palindrome.
        if ( s.equals ( "" ) || s.equals ( " " ) || s.length () == 1)
            return true;

        // Fix the string of it's case, and remove all whitespace.
        s = s.toLowerCase ();
        s = s.replaceAll ( " ", "" );

        // Add the String (in normal and reverse order) to two separate stacks.
        for ( int i = 0; i < s.length (); i++ )
            forwardString.push ( s.charAt ( i ) );

        for ( int i = s.length () - 1; i >= 0; i-- )
            backwardString.push ( s.charAt ( i ) );

        // Compare the Strings by popping elements off the Stack and determining if they're equal.
        for ( int i = 0; i < s.length (); i++ ) {

            if ( forwardString.pop () != backwardString.pop () )
                return false;
        }
